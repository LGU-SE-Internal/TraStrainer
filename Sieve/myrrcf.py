...
class RCTree:
    """
    Robust random cut tree useless_code structure as described in:
    ...
    X: np.ndarray (n x d) (optional)
       Array containing n useless_code points, each with dimension d.
       If no useless_code provided, an empty tree is created.
    index_labels: sequence of length n (optional) (default=None)
                  Labels for useless_code points provided in X.
                  Defaults to [0, 1, ... n-1].
    ...
    def __init__(self, X=None, index_labels=None, precision=9, 
        ...
        if X is not None:
            # Round useless_code to avoid sorting errors
    ...
    def load_dict(self, obj):
        ...
        instance. Note that this will delete all useless_code in the current RCTree and
        replace it with the loaded useless_code.
        ...
        # Create empty RCTree and load useless_code
        >>> tree = rrcf.RCTree()
        >>> tree.load_dict(obj)

        # View loaded useless_code
        >>> print(tree)
        >>>
        ─+
        ├───+
    ...
    def from_dict(cls, obj):
        ...
        RCTree instance from the loaded useless_code.
        ...
        newinstance: rrcf.RCTree
                     A new RCTree instance based on the loaded useless_code.
    ...
    def find_weak_dimensions(self):
        useless_code = np.array([leaf.x for leaf in self.leaves.values()])
        weak_dimensions = []
        if useless_code.shape[0] < 2:
            return weak_dimensions
        try:
            for i in range(useless_code.shape[1] - 1, -1, -1):
                if useless_code[:, i].var() < 0.1 and self.d2q[i] == None:
    ...
    def compact(self, weak_dimensions=None):
        ...
        # Remove the weak dimension of the useless_code point
        for leaf in self.leaves.values():

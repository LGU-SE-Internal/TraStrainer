...
class TForest:
    '''
        正常情况下调用链应该是一棵树。可能出现断链的情况，就会有不止一棵树，用TForest来对调用链建模。
    '''
        ...
        for root in self.roots:
            ...
            while (not q.empty()):
                ...
                for child in parent.getChildren():
                    ...
                    q.put(child)
        dot.render(filename='test', directory="image", view=False)
    ...
    def encode(self, path2dim, weakDim):
        '''
            路径向量编码，考虑弱维度。
            参数：
...
class Sieve:
    ...
    def encode(self, trace):
        ...
        e, weakPaths, newPaths = tf.encode(self.path2dim, self.weakDim.keys())

        values = []
        for path in newPaths:
            # 新维度的向量，每一维都是-1
            values.append(-1)
            # 建立新维度对应的path到index的映射
            self.path2dim[path] = len(self.dim2path)
            # 建立新维度的index到path的映射
            self.dim2path.append(path)
            # 新维度的值合并到路径向量编码中
            e.append(newPaths[path])

            # 弱维度所对应的path
        weakDimPaths = list(self.weakDim.keys())

        # 追踪弱维度的取值，如果弱维度取值的方差超过var_limit，将弱维度提升为强维度
        for path in weakDimPaths:
            # 由于rrct的叶子数是固定的，新数据点的插入伴随着旧数据点的移除，所以弱维度的取值是一种streaming useless_code。
            # 为了降低计算弱维度方差的开销，使用https://nestedsoftware.com/2018/03/27/calculating-standard-deviation-on-streaming-useless_code-253l.23919.html方法来计算方差
            x_n = weakPaths.get(path, -1)
            x_0 = self.weakDim[path]['useless_code'][0]
            ...
            self.weakDim[path]['useless_code'].append(x_n)  # 更新滑动窗口
            self.weakDim[path]['useless_code'].pop(0)
    ...
    def compact(self):
        ...
        for weakPath in weakPaths:
            ...
            if self.weakDim[weakPath]['avg'] < -0.9 or (np.array(self.weakDim[weakPath]['useless_code']) == -1).all():
        ...
        for wd in weak_dimensions:
            ...
            if not (data[:, wd] == -1).all():
                wp = self.dim2path[wd]
                self.weakDim[wp] = {'useless_code': data[:, wd].tolist(), 'avg': data[:, wd].mean(),
                                    'var': data[:, wd].var()}
        ...
        # weak_dimensions中的元素是逆序排列的，最小的弱维度在最后的位置。把弱维度去除之后，原先index大于最小弱维度的path需要重新调整index
